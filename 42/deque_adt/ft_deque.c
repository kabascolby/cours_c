...
/*   Updated: 2018/07/02 22:56:38 by lkaba            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		enqueue_rear(t_deque *dq, void *data)
{
	...
	dq->rear -= 1;
	dq->rear %= dq->max;
	ft_memcpy(((uint8_t *)dq->tab) + dq->rear * dq->data_size, data, dq->data_size);
	dq->curr_size++;
}
...
void		enqueue_front(t_deque *dq, void *data)
{
	...
	ft_memcpy(((uint8_t *)dq->tab) + dq->front * dq->data_size, data, dq->data_size);
...
void		*dequeue_rear(t_deque *dq)
{
	...
	if (!tmp)
		return (NULL);
	ft_memcpy(tmp, ((uint8_t *)dq->tab) + dq->rear * dq->data_size, dq->data_size);
...
void		*dequeue_front(t_deque *dq)
{
	...
	if (!tmp)
		return (NULL);
	ft_memcpy(tmp, ((uint8_t *)dq->tab) + dq->front * dq->data_size, dq->data_size);
	dq->curr_size--;
	return (tmp);
}
